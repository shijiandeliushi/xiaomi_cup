...
#ifndef __microstrain_lcmt_hpp__
#define __microstrain_lcmt_hpp__

#include <lcm/lcm_coretypes.h>



class microstrain_lcmt
{
    public:
        float      quat[4];
...
int microstrain_lcmt::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int microstrain_lcmt::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int microstrain_lcmt::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t microstrain_lcmt::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* microstrain_lcmt::getTypeName()
{
    return "microstrain_lcmt";
}

int microstrain_lcmt::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->rpy_imu[0], 3);
    if(tlen < 0) return tlen; else pos += tlen;
...
int microstrain_lcmt::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->rpy_imu[0], 3);
    if(tlen < 0) return tlen; else pos += tlen;
...
int microstrain_lcmt::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t microstrain_lcmt::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0x5cd056dfbad25aebLL;
