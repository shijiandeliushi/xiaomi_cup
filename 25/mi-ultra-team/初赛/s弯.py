...
class LineFollower(Node):
    def __init__(self):
        ...
        print("reliability: ", camera_qos.reliability)
    ...
    def process_image(self, img):
        ...
        if contours:
            # --- 新：先把每个轮廓 BoundingRect 的中心 x 算出来
            rects = [cv2.boundingRect(cnt) for cnt in contours]
            centers = [(x + w/2.0) for (x, _, w, _) in rects]
            # 根据离图像中心（self.target_x）的距离从小到大排序
            sorted_idx = sorted(range(len(centers)), key=lambda i: abs(centers[i] - self.target_x))
            # 取最近的两条“线”
            sel = sorted_idx[:2]
            contours = [contours[i] for i in sel]
            rects    = [rects[i]    for i in sel]
            ...
            if self.pixel_per_cm is not None:
               ...
               # 注意同时更新 rects／centers，否则下面计算会错
                tmp = []
                for cnt, (x,y,w,h) in zip(contours, rects):
                   if w >= min_width_px:
                       tmp.append((cnt,(x,y,w,h)))
                contours, rects = zip(*tmp) if tmp else ([],[])
            # if len(contours) == 2:
            #     # 计算左右线的像素位置
            ...
            if len(contours) == 2:
                # 既然我们已经有 rects，就直接用它
                (x1, _, w1, _), (x2, _, w2, _) = rects
                print(f"x1: {x1}, x2: {x2}, w1: {w1}, w2: {w2}")
                left_edge  = min(x1, x2)
                ...
                if self.pixel_per_cm is None and pixel_dist > 0:
                    ...
                    self.pixel_per_cm = pixel_dist / 80.0  # 手动调整
                ...
                if abs(left_edge - right_edge) > 10:
                    ...
                    if curvature > 0.8:  # 曲率较大时动态调整中心线预测
         ...
         # 只检测到一条线，使用标定后的偏移补全
                ...
                if edge_center < self.image_width/2:
                    ...
                    print(f"[Process] 丢右侧，中心在右侧 center_x={center_x:.1f}")
                else:
                    # 丢左侧，中心在左侧
                    center_x = edge_center - self.track_width_cm * self.pixel_per_cm
                    print(f"[Process] 丢左侧，中心在左侧 center_x={center_x:.1f}")
    ...
    def _draw_debug_overlay(self, img, center_x, error):
        ...
        cx = int(center_x)
        cy = self.roi_y + self.roi_height // 2
        cv2.circle(debug, (cx, cy), 20, (255, 0, 0), -1)
        # 目标线
        cv2.line(debug,
                 (self.target_x, self.roi_y),
                 (self.target_x, self.roi_y + self.roi_height),
        ...
        def navigate_track():
            ...
            while rclpy.ok():
                ...
                if not detected:
                    ...
                    # 正常循迹控制
                    turn_speed = line_follower.pid_control(error)
                    forward_speed = 0.05 * (1.0 - 0.5 * abs(turn_speed))  # 调整速度曲线
